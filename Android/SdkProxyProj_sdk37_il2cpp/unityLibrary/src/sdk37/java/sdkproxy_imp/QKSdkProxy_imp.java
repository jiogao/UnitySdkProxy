...
public class QKSdkProxy_imp extends QKBaseSdkProxy {
    ...
    public void Pay(String strData, QKUnityBridgeManager.QKUnityCallbackFunc callback) {
        ...
        try {
            ...
            try {
                ...
                order.setRealPayMoney(Float.parseFloat(price));
            }
            ...
            GameUSDK.getInstance().pay(context, order, new GameUPayListener() {
                ...
                public void payFail(int code, String msg) {
                    Log.i("QKSdkProxy_imp","Pay fail "+msg);
    ...
    //<editor-fold desc="Sdk37">
    public void Sdk37_getPlatformId(String strData,QKUnityBridgeManager.QKUnityCallbackFunc callback) {
        Log.i("QKSdkProxy_imp","Sdk37_GetPlatformId");
        /**
         * 获取渠道标识，默认返回 "37h5"，需要在 init() 接口之后调用
         */
        String platform_appid = GameUSDK.getInstance().getPlatformId();
        callback.Invoke(platform_appid);
    }

    public void Sdk37_getChildGameId(String strData,QKUnityBridgeManager.QKUnityCallbackFunc callback) {
        Log.i("QKSdkProxy_imp","Sdk37_GetChildGameId");
        /**
         * 获取子游戏ID，需要在 init() 接口之后调用
         */
        String childGameId = GameUSDK.getInstance().getChildGameId();
        callback.Invoke(childGameId);
    }

    public void Sdk37_getClientId(String strData,QKUnityBridgeManager.QKUnityCallbackFunc callback) {
        Log.i("QKSdkProxy_imp","Sdk37_GetClientId");
        /**
         * 获取clientId，需要收到登录成功回调后调用
         */
        String clientId = GameUSDK.getInstance().getClientId();
        callback.Invoke(clientId);
    }

    public void Sdk37_queryAntiAddiction(String strData,QKUnityBridgeManager.QKUnityCallbackFunc callback) {
        Log.i("QKSdkProxy_imp","Sdk37_queryAntiAddiction");
        /**
         * 查询当前登录用户的实名状态
         */
        GameUSDK.getInstance().queryAntiAddiction(context, new GameURealNameListener() {
            @Override
            public void onResult(int authCode, String authMsg) {
                String verifiedInfo = " \n authCode:" + authCode + "\n authMsg:" + authMsg;
                ToastUtil.toast(context, verifiedInfo);
                if (authCode == CODE_ADULT_CODE_NO_REALNAME){
                    ToastUtil.toast(context, "未实名认证");
                }else if (authCode == CODE_ADULT_CODE_REALNAME_NO_ADULT){
                    ToastUtil.toast(context, "已实名认证，用户未成年");
                }else if (authCode == CODE_ADULT_CODE_REALNAME_ADULT){
                    ToastUtil.toast(context, "已实名认证，用户已成年");
                }
            ...
            public void onError(int code, String errMsg) {
                Log.e("GameUSDK", "onError,code=" + code + ",msg:" + errMsg);
                ToastUtil.toast(context, "实名失败：\n" + errMsg);
    ...
    public  void  Sdk37_getGameId(String strData,QKUnityBridgeManager.QKUnityCallbackFunc callback){

        callback.Invoke(game_id);
    }
    //</editor-fold>
    ...
    public void onRequestPermissionsResult(int requestCode, String[] permissions, int[] grantResults) {
        ...
        GameUSDK.getInstance().onRequestPermissionsResult(requestCode, permissions, grantResults);
    }
    //</editor-fold>
